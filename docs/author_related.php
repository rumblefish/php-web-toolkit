...
try {
    ...
    switch($rest->getVerb()) {
        case 'get':
			...
			if($rest->getVars('authName')) {

				$query = "SELECT SQL_CALC_FOUND_ROWS s.prodID AS rowID, s.code, s.title, s.price, s.stock, s.shortdesc, s.longdesc, s.author, LOWER(REPLACE(REPLACE(REPLACE(s.author, \"'\", ''), '/', ''), ' ', '')) as shortname, s.lang, s.size AS type, s.minpage AS size, s.discprice, IF(s.image='',s.image, CONCAT('".$_SERVER['SERVER_NAME']."/products/125x150/',s.image)) AS trackBackImage, CONCAT('".$_SERVER['SERVER_NAME']."/view/',s.prodID,'/prodlisttitle/') AS trackBackUrl FROM store_prods AS s WHERE s.active = 'Yes' AND s.image != '' AND s.author LIKE ? ORDER BY s.title ASC";
				$params = array();

                $params[] = '%'.stripslashes($rest->getVars('authName')) .'%';
